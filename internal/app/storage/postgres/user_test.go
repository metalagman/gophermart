...
func TestUserRepository_createTables(t *testing.T) {
	mdb, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
	...
	mock.ExpectExec(`CREATE TABLE`).WillReturnResult(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := r.createTables(); (err != nil) != tt.wantErr {
...
func TestUserRepository_createTables_Fail(t *testing.T) {
	mdb, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
	...
	mock.ExpectExec(`CREATE TABLE`).WillReturnError(errors.New("you shall not pass"))
